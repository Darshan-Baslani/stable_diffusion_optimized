...
class DDPMSampler:
    def __init__(
        ...
        self.betas = (
            torch.linspace(
                beta_start**0.5, beta_end**0.5, num_triaining_steps, dtype=torch.float32
            )
            ** 2
        )
        ...
        self.timesteps = torch.from_numpy(
            np.arange(0, num_triaining_steps)[::-1].copy()
        )
    ...
    def step(self, timestep: int, latents: torch.Tensor, model_output: torch.Tensor):
        ...
        pred_original_sample = (
            latents - (beta_t_bar**0.5) * model_output
        ) / alpha_t_bar**0.5

        # eq 7 for mean and variance
        first_term = (
            (alpha_t_bar_prev**0.5 * current_beta_t) / beta_t_bar
        ) * pred_original_sample
    ...
    def add_noise(
        self, original_samples: torch.FloatTensor, timesteps: torch.Tensor
    ) -> torch.Tensor:
        # eq 4 ddpm paper

        alpha_t_bar = self.alpha_t_bar.to(
